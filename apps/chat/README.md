...
- **Engine** (`src/engine.js`): Session state, load STATE + MEM–RELEVANCE for entity, build prompt, call OpenAI, parse response into the **canonical shape** `{ text, options, entity }`. All channels consume only this shape; see [docs/RESPONSE–FORMAT.md](docs/RESPONSE–FORMAT.md).
...
4. **Telegram**  
   Open your bot in Telegram and send e.g. `Russia update` or `Switch to Persia`. You get a reply and 8 option buttons (A–H). Tapping a button runs that option and returns the next reply + options.
...
Session keeps current entity (e.g. RUSSIA, PERSIA). User can say “Russia update”, “Iran update”, “Switch to Persia” to change it. Default is RUSSIA.
