CIV–INDEX–PERSIA — v3.20
Civilizational Memory Codex · Index File
Repository: https://github.com/rbtkhn/civilization_memory

Status: ACTIVE · CANONICAL
Version: 3.20
Supersedes: CIV–INDEX–PERSIA v3.19
Upgrade Type: ADDITIVE · MEM–PERSIA–LAW–MONARCHY
Civilization: PERSIA
Class: CIV–INDEX
Governed by: CIV–MEM–CORE v3.0
Template: CIV–INDEX–TEMPLATE v3.0
Last Update: February 2026

MEM COUNT: 76 MEM–PERSIA files (reconciled 2026-02)

────────────────────────────────────────────────────────────
I. INDEX PURPOSE & GOVERNANCE
────────────────────────────────────────────────────────────
This file serves as the **authoritative index** for all MEM–PERSIA files
within the Civilizational Memory Codex.

Functions:
• Provide a complete, auditable registry of Persian civilizational memory
• Enforce cross-civilizational comparability with other CIV indexes
• Prevent duplication, drift, or category imbalance
• Serve as the primary navigation spine for Scholar Mode

Index rules:
• Every MEM–PERSIA file must be listed here
• Category naming must remain consistent across civilizations
• New entries require index update to be considered canonical

────────────────────────────────────────────────────────────
II. CANONICAL GOVERNANCE FILES
────────────────────────────────────────────────────────────
• CIV–CORE–PERSIA — v3.3 (Primary civilizational engine)
• CIV–SCHOLAR–PERSIA — v2.0 (Learning and synthesis)
• CIV–ARC–PERSIA — v2.0 (Academic Reference Canon)

────────────────────────────────────────────────────────────
III. DYNASTIC MEMORY FILES
────────────────────────────────────────────────────────────
• MEM–PERSIA–EMPIRE.md  
  (Empire as recurring institutional form; sacral kingship; parity and reconstitution; Achaemenid–Sasanian sequence)

• MEM–PERSIA–ACHAEMENID–DYNASTY.md  
  (Imperial universalism, tolerance, administrative depth)

• MEM–PERSIA–SASANIAN–DYNASTY.md  
  (Siege-state consolidation, sacral kingship, parity war)

• MEM–PERSIA–PARTHIAN.md  
  (Arsacid; confederal governance; denial warfare; Roman parity; Carrhae; 247 BC–224 AD)

• MEM–PERSIA–SELEUCID.md  
  (Hellenistic interlude; Macedonian-Greek rule over plateau and Mesopotamia; 312–247 BC east; template transfer to Parthian)

• MEM–PERSIA–MEDES.md  
  (Pre-imperial hegemony; Persian–Median fusion; Ecbatana)

• MEM–PERSIA–SATRAPIES.md  
  (Satrapal system; tribute; modular imperial control)

• MEM–PERSIA–SAFAVID–DYNASTY.md  
  (Shi'a state formation; Isfahan; Ottoman rivalry; post-Mongol reconstitution; 1501–1722)

• MEM–PERSIA–QAJAR.md  
  (Tehran capital; Russian and Ottoman pressure; territorial loss; Great Game; 1789–1925)

────────────────────────────────────────────────────────────
IV. FOUNDATIONAL FIGURES & IDEOLOGICAL ORIGINS
────────────────────────────────────────────────────────────
• MEM–PERSIA–ACHAEMENID–CYRUS–II.md  
  (Imperial founder, tolerance doctrine, sovereignty-with-restraint)

• MEM–PERSIA–ZOROASTER.md  
  (Moral–cosmic legitimacy engine, endurance theology)

• MEM–PERSIA–ACHAEMENID–DARIUS–I.md  
  (Imperial systematizer; satrapal design; Royal Road; Behistun)

────────────────────────────────────────────────────────────
IV.A. SCIENCE (SCI SUBCLASS)
────────────────────────────────────────────────────────────
• MEM–PERSIA–SCI–AL–BIRUNI.md  
  (973–1048; geodesy, India; precision and cross-civilizational inquiry; ARC)
• MEM–PERSIA–SCI–AVICENNA.md  
  (980–1037; Canon, Healing; synthesis; Persianate court)
• MEM–PERSIA–SCI–RAZI.md  
  (854–925; Rey; medicine, smallpox/measles; empirical method)
• MEM–PERSIA–SCI–OMAR–KHAYYAM.md  
  (1048–1131; algebra, Jalali calendar, Rubaiyat; order and voice)
• MEM–PERSIA–SCI–NASIR–AL–DIN–TUSI.md  
  (1201–1274; Maragheh observatory; trigonometry; post-Mongol reconstitution)
• MEM–PERSIA–SCI–AL–KHWARIZMI.md  
  (c. 780–850; algebra, algorithms; Khwarezm, Baghdad)
• MEM–PERSIA–SCI–MIRZAKHANI.md  
  (1977–2017; Fields Medal; moduli spaces; modern, diaspora)

────────────────────────────────────────────────────────────
V. RELIGIOUS & REGIME TRANSFORMATION FILES
────────────────────────────────────────────────────────────
• MEM–PERSIA–ISLAM.md  
  (Civilizational absorption of Islamic theology and law)

• MEM–PERSIA–IRAN.md  
  (Iran as modern state container; name, territory, constitutional sequence; 20th–21st c.)

• MEM–PERSIA–IRAN–ISLAMIC–REVOLUTION.md  
  (1979 revolution; causes, sequence, outcome; sovereignty restoration encoding)

• MEM–PERSIA–IRAN–ISLAMIC–REPUBLIC.md  
  (IRI regime 1979–present; dual structure, velayat-e faqih, siege adaptation)

• MEM–PERSIA–ISLAMIC–REPUBLIC.md  
  (Modern sovereignty–siege regime, clerical–state fusion; complements IRAN–ISLAMIC–REPUBLIC)

• MEM–PERSIA–IRAN–MOSSADEGH.md  
  (Prime minister; oil nationalization; 1953 coup; sovereignty rupture; memory fueled 1979)

────────────────────────────────────────────────────────────
V-A. LITERATURE & CIVILIZATIONAL TRANSMISSION
────────────────────────────────────────────────────────────
• MEM–PERSIA–LIT–FERDOWSI.md  
  (Shahnameh; epic carrier of royal memory; Persian as language of kingship; ARC–PERSIA-T-MEDIEVAL anchor)

• MEM–PERSIA–LIT–HEDAYAT.md  
  (The Blind Owl; modern prose breakthrough; alienation, modernity as trauma; Pahlavi period)

• MEM–PERSIA–LIT–NIMA–YUSHIJ.md  
  (Founder of she'r-e now; new poetry; broke classical metre; bridge tradition and modernity)

────────────────────────────────────────────────────────────
V-B. LAW (LAW SUBCLASS)
────────────────────────────────────────────────────────────
• MEM–PERSIA–LAW–SIYASATNAMA.md  
  (Nizam al-Mulk; Book of Government; Seljuk governance as law; ARC-T-MEDIEVAL)
• MEM–PERSIA–LAW–CONSTITUTION.md  
  (1906 Mashruta and 1979 IRI constitution; written constraint and dual structure)
• MEM–PERSIA–LAW–SASANIAN.md  
  (Sasanian/Zoroastrian codified law; asha; sacral kingship; Madigan)

• MEM–PERSIA–LAW–MONARCHY.md  
  (Monarchy as legal form; kingship, succession; 1906 limit; 1979 abolition)

────────────────────────────────────────────────────────────
VI. GEOGRAPHIC & SPATIAL MEMORY FILES
────────────────────────────────────────────────────────────
• MEM–PERSIA–GEO–IRANIAN–PLATEAU.md  
  (Fortress core; elevated interior; ring of mountains; defense-dominant; CORE cyber–plateau)

• MEM–PERSIA–GEO–CASPIAN.md  
  (Northern buffer, energy corridor, strategic depth modifier)

• MEM–PERSIA–GEO–PERSIAN–GULF.md  
  (Strait of Hormuz chokepoint; littoral asymmetry; energy corridor)

• MEM–PERSIA–GEO–INDIAN–OCEAN.md  
  (Peripheral access; trade corridor; Gulf egress; Nearchus route)

• MEM–PERSIA–GEO–LEVANT.md  
  (Western frontier; Roman–Persian collision zone; reach without hold)

• MEM–PERSIA–GEO–MESOPOTAMIA.md  
  (Fluvial corridor; Ctesiphon; western lowland; plateau approach; CORE Caspian–Caucasus–Mesopotamia)

• MEM–PERSIA–GEO–CAUCASUS.md  
  (Northern land barrier; passes; Russian territorial loss; Golestan, Turkmanchay; buffer and cession)

• MEM–PERSIA–SILK–ROAD.md  
  (Persian nexus; transit power; intermediary advantage; Royal Road)

• MEM–PERSIA–ARMENIA.md  
  (Roman–Persian buffer; Iranian sphere; client oscillation)

• MEM–PERSIA–PERSEPOLIS.md  
  (Achaemenid ceremonial capital; Apadana; architectural propaganda)

• MEM–PERSIA–PASARGADAE.md  
  (Cyrus's capital and tomb; first Achaemenid ceremonial site; Alexander spared tomb)

• MEM–PERSIA–EGYPT.md  
  (Achaemenid satrapy; Cambyses; incorporation, revolt)

• MEM–PERSIA–LYDIA.md  
  (Croesus; Cyrus's western conquest; Royal Road origin; Sardis)

• MEM–PERSIA–BABYLON.md  
  (Cyrus's conquest; liberation narrative; Cyrus Cylinder)

• MEM–PERSIA–TEHRAN.md  
  (Capital from 1786; seat of power; revolution locus; Qajar–Pahlavi–IRI)

• MEM–PERSIA–QOM.md  
  (Holy city; shrine of Fatima Masumeh; hawza; clerical legitimacy; Tehran–Qom axis)

• MEM–PERSIA–ISFAHAN.md  
  (Safavid capital; Naqsh-e Jahan; "half the world"; architectural propaganda)

────────────────────────────────────────────────────────────
VII. IMPERIAL SHOCK & FOREIGN RULE FILES
────────────────────────────────────────────────────────────
• MEM–PERSIA–MONGOL–EMPIRE.md  
  (Catastrophic rupture, administrative memory survival)

────────────────────────────────────────────────────────────
VIII. WAR & CIVILIZATIONAL CONFLICT FILES
────────────────────────────────────────────────────────────
WAR corpus (integrated, v2.0): Ionian Revolt → Marathon → Thermopylae → Salamis (Greek
wars); Scythian (northern denial); Alexander (tempo/conquest); Carrhae
(founding Roman–Persian); Rome (seven-century parity). Cross-refs in MEM
CONNECTIONS. IONIAN–REVOLT, XERXES, WAR–SCYTHIAN, WAR–MARATHON, WAR–THERMOPYLAE,
WAR–ROME at v2.0; GEO–CASPIAN v2.0 (Scythian integration).

• MEM–PERSIA–WAR–ALEXANDER.md  
  (Decapitation shock, endurance vs charisma conquest)

• MEM–PERSIA–WAR–CARRHAE.md  
  (Archetypal Parthian denial victory; mobility over legion)

• MEM–PERSIA–WAR–ROME.md  
  (Parity war, endurance doctrine validation, exhaustion limit)

• MEM–PERSIA–WAR–MARATHON.md  
  (490 BC; Persian expedition; encoding divergence; Llewellyn-Jones)

• MEM–PERSIA–WAR–SCYTHIAN.md  
  (Darius's northern campaign; nomadic denial; limits of Persian power)

• MEM–PERSIA–IONIAN–REVOLT.md  
  (499–493 BC; satrapal rebellion; Athenian involvement; Marathon causation)

• MEM–PERSIA–WAR–THERMOPYLAE.md  
  (480 BC; Xerxes; Greek stand; Persian tactical victory)

• MEM–PERSIA–WAR–SALAMIS.md  
  (480 BC; naval; Greek coalition victory; encoding divergence)

• MEM–PERSIA–WAR–IRAN–IRAQ.md  
  (1980–1988; invasion, stalemate; martyrdom engine; CORE continuity node)

────────────────────────────────────────────────────────────
IX. IMPERIAL FIGURES
────────────────────────────────────────────────────────────
Achaemenid:
• MEM–PERSIA–ACHAEMENID–CAMBYSES–II.md  
  (Egyptian conquest; succession crisis; Herodotean encoding)
• MEM–PERSIA–ACHAEMENID–XERXES.md  
  (Greek wars; Thermopylae, Salamis; Persian encoding)
• MEM–PERSIA–ACHAEMENID–DARIUS–III.md  
  (Terminal Achaemenid; Issus, Gaugamela; legitimacy collapse)

Sasanian:
• MEM–PERSIA–SASANIAN–ARDASHIR–I.md  
  (Sasanian founder; restoration ideology; Hormozdgan)

• MEM–PERSIA–SASANIAN–SHAPUR–I.md  
  (King of Kings; captor of Valerian; triumphal propaganda)

• MEM–PERSIA–SASANIAN–KHOSROW–II.md  
  (Khosrow II; terminal Roman–Persian war; overextension; mutual exhaustion)

────────────────────────────────────────────────────────────
X. EXTERNAL ACTORS (ROMAN–PERSIAN THEATER & BEYOND)
────────────────────────────────────────────────────────────
• MEM–PERSIA–ROMAN–EMPIRE.md  
  (Roman Empire as parity rival in Persian memory; Rūm; trade, legitimacy contest, exhaustion)

• MEM–PERSIA–OTTOMAN–EMPIRE.md  
  (Ottoman as western Islamic rival; Sunni–Shi'a contest; Safavid–Ottoman wars; territorial stalemate; Qajar dual pressure)

• MEM–PERSIA–PALMYRA.md  
  (Post-Valerian counterweight; Odaenathus, Zenobia; denial of Persian consolidation)

• MEM–PERSIA–ARABIA.md  
  (Peninsula in Persian memory; conquest source; Gulf southern littoral; Lakhmid frontier)

• MEM–PERSIA–INDIA.md  
  (Eastern satrapies, trade corridor, Persianate exchange; no parity war)

• MEM–PERSIA–RUSSIA.md  
  (Northern pressure; territorial loss; Great Game; Soviet and contemporary)

• MEM–PERSIA–OMAN.md  
  (Strait partner; Gulf of Oman; Musandam; maritime trade; southern littoral; diplomatic channel)

• MEM–PERSIA–CHINA.md  
  (Silk Road eastern terminus; trade corridor; no parity war; modern partnership; axis optionality)

• MEM–PERSIA–BRITISH–EMPIRE.md  
  (Great Game southern pressure; concessions; 1953 coup; colonial trauma; sovereignty denied)

• MEM–PERSIA–IRAQ.md  
  (Mesopotamian successor state; Iran–Iraq War; Ba'ath; contested corridor; invader and neighbor)

• MEM–PERSIA–SYRIA.md  
  (Levantine theater; conquest and reversal; Khosrow then Heraclius; modern Iran–Syria alliance; axis of resistance)

────────────────────────────────────────────────────────────
XI. CATEGORY BALANCE CHECK (v1.1)
────────────────────────────────────────────────────────────
Dynasty:        PRESENT (8: Empire, Achaemenid, Sasanian, Parthian, Medes, Satrapies, Safavid, Qajar)
Foundational:   PRESENT (3: Cyrus, Zoroaster, Darius I)
Geography:      PRESENT (17: Iranian Plateau, Caspian, Persian Gulf, Indian Ocean, Levant, Mesopotamia, Caucasus, Silk Road, Armenia, Persepolis, Pasargadae, Egypt, Lydia, Babylon, Tehran, Qom, Isfahan)
Religion/Regime: PRESENT (7: Islam, Iran, Iran–Revolution, Iran–IRI, Islamic Republic, Iran–Mossadegh)
Imperial Shock: PRESENT (1: Mongol)
War/Conflict:   PRESENT (8: Alexander, Carrhae, Rome, Marathon, Scythian, Ionian Revolt, Thermopylae, Iran–Iraq)
Imperial Figs:  PRESENT (6: Cambyses, Xerxes, Darius III, Ardashir, Shapur, Khosrow II)
External:       PRESENT (11: Roman Empire, Ottoman Empire, Palmyra, Arabia, India, Russia, Oman, China, British Empire, Iraq, Syria)
Literature:     PRESENT (3: Ferdowsi, Hedayat, Nima Yushij)

Total MEM–PERSIA: 63 | GEO–MEM: 8
No critical category gaps detected.

────────────────────────────────────────────────────────────
XII. CONTINUITY NOTES
────────────────────────────────────────────────────────────
• Persia's memory corpus emphasizes endurance, sovereignty, and siege logic
• Dynastic and ideological layers are balanced, not personality-heavy
• War memory prioritizes parity and exhaustion over conquest narratives
• Religious memory functions as legitimacy infrastructure, not belief alone

────────────────────────────────────────────────────────────
END OF FILE — CIV–INDEX–PERSIA — v3.17
────────────────────────────────────────────────────────────

