...
export default function ScholarInterface({ mode, scholarContent, loadedMemFiles = [], civilization, coreContent, indexContent, doctrineContent, onMemFileUsed, onFileContentUpdate, isMemFileLocked = false, onUnlockRequest, allFiles = [], onSwitchMemFile, onLoadMemFileToLLM, onUnloadMemFile, onClearLoadedFiles }: ScholarInterfaceProps) {
  ...
  const generateContextualOptions = useCallback((
    ...
    // If audit shows non-compliance and we have a loaded MEM file
    if (isAuditResponse && isNonCompliant && loadedMemFiles.length > 0) {
      const memFile = loadedMemFiles[loadedMemFiles.length - 1];
      const memFileName = memFile.path.split('/').pop() || memFile.path;
      
      // Always show upgrade option first for non-compliant files (regardless of mode)
      ...
      // If in WRITE mode, add more options
      if (mode === 'WRITE') {
        options.push({ letter: 'c', text: `Continue editing ${memFileName}` });
        options.push({ letter: 'd', text: `Analyze content and structure of ${memFileName}` });
      } else {
        // If not in WRITE mode, suggest switching
        options.push({ letter: 'c', text: `Switch to WRITE mode to upgrade ${memFileName}` });
        options.push({ letter: 'd', text: `Analyze content and structure of ${memFileName}` });
      }
    }
    // If audit shows compliance
    else if (isAuditResponse && !isNonCompliant && loadedMemFiles.length > 0) {
      const memFile = loadedMemFiles[loadedMemFiles.length - 1];
      const memFileName = memFile.path.split('/').pop() || memFile.path;
      
      options.push({ letter: 'a', text: `View detailed compliance report for ${memFileName}` });
      options.push({ letter: 'b', text: `Analyze content and structure of ${memFileName}` });
      options.push({ letter: 'c', text: `Find connections to other MEM files from ${memFileName}` });
    }
    // If we have a loaded MEM file, show standard options
    else if (loadedMemFiles.length > 0) {
      const memFile = loadedMemFiles[loadedMemFiles.length - 1];
      const memFileName = memFile.path.split('/').pop() || memFile.path;
      const civName = civilization?.toUpperCase() || 'CIV';
      
      options.push({ letter: 'a', text: `Audit ARC–${civName} compliance for ${memFileName}` });
      options.push({ letter: 'b', text: `Audit MEM–TEMPLATE compliance for ${memFileName}` });
      options.push({ letter: 'c', text: `Analyze content and structure of ${memFileName}` });
      options.push({ letter: 'd', text: `Find connections to other MEM files from ${memFileName}` });
      options.push({ letter: 'e', text: `Compare ${memFileName} with related MEM files` });
      options.push({ letter: 'f', text: `Extract key themes and patterns from ${memFileName}` });
    }
    // Default options when no MEM file is loaded
    else {
      options.push({ letter: 'a', text: 'Load a MEM file to analyze' });
      options.push({ letter: 'b', text: 'List available MEM files' });
      if (mode === 'WRITE') {
        options.push({ letter: 'c', text: 'Create a new MEM file' });
      }
    }
  ...
  useEffect(() => {
    if (loadedMemFiles.length > 0 && civilization && !isProcessing) {
      ...
      if (currentLoadedFile !== lastLoadedMemFileRef.current) {
        ...
        // Create and store the standard options message
        const optionsMessage = `MEM file loaded: ${memFileName}\n\nWhat would you like to do?\n\na) Audit ARC–${civName} compliance\nb) Audit MEM–TEMPLATE compliance\nc) Analyze content and structure\nd) Find connections to other MEM files\ne) Compare with related MEM files\nf) Extract key themes and patterns`;
